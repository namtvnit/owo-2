...
import {getResult, listCheckbox, trueFalse} from "./prompt.mjs";
import {log} from "./console.mjs";
import {DataPath} from "../index.mjs";
...
var aliasName, timePray, userIdPray, huntPet, textOwo;
...
function listAccount(data) {
    ...
    obj.filter = (value) => {
        ...
        if (user) {
            return Buffer.from(user.token.split(".")[0], "base64").toString();
        } else {
            return value;
        }
    }
...
function getAliasName(cache) {
    const obj = {
        type: "input",
        message: "Enter Your Alias Name Owo: "
    };
    if (cache) {
        obj.default = cache;
    }
...
function getTimePray(cache) {
    const obj = {
        type: "input",
        message: "Continue time Pray: "
    };
    if (cache) {
        obj.default = cache;
    }
...
function getUserPray(cache) {
    const obj = {
        type: "input",
        message: "User Id pray: "
    };
    if (cache) {
        obj.default = cache;
    }
...
function listGuild(cache) {
    const obj = listCheckbox("list", "Select a guild to farm", client.guilds.cache.map(guild => ({
        name: guild.name,
        value: guild.id
    })))
    if (cache && client.guilds.cache.get(cache)) {
        obj.default = () => {
...
function listChannel(cache) {
    ...
    const obj = listCheckbox("checkbox", "Select channels to farm (Farming Channel will be changed randomly if multiple channels are selected)", [{
        name: "Back to guilds select",
        value: -1
    }, ...channelList.map(ch => ({name: ch.name, value: ch.id}))])
    obj.validate = (ans) => {
        return ans.length > 0 ? true : "Please Choose At Least One Channel"
    }
    if (cache && channelList.some(cn => cache.indexOf(cn.id) >= 0)) {
        obj.default = [...channelList.filter(channel => cache.indexOf(channel.id) >= 0).map(channel => channel.id)];
    }
    return obj;
}
        ...
        async validate(ans) {
            if (waynotify.includes(1) || waynotify.includes(2)) {
                if (ans.match(/^\d{17,19}$/)) {
                    if (ans == client.user.id) {
                        return "Selfbot account ID is not valid for Call/Direct Message option"
                    }
                    const target = client.users.cache.get(ans);
                    if (!target) {
                        return "User not found";
                    }
                    if (target.relationships == "FRIEND") {
                        return true;
                    ...
                    } else if (target.relationships == "PENDING_OUTGOING") {
...
function music1(cache) {
    const obj = {
        ...
        validate(answer) {
            if (!answer.match(/^([a-zA-Z]:)?(\/?[^\/\0]+)+(\/[^\/\0]+)?$/)) {
                return "Invalid Directory";
            }
            const supportedAudioExtensions = ['.wav', '.mp3', '.m4a', '.flac', '.ogg', '.aac'];
            if (!fs.existsSync(answer)) {
                return "Could Not Find That Path";
            }
            const stats = fs.statSync(answer)
            if (stats.isDirectory()) {
                if (fs.readdirSync(answer).some(file => supportedAudioExtensions.includes(path.extname(path.join(answer, file))))) {
                    return true;
                } else {
                    return "No Supported File Found In That Directory"
                }
            }
            if ((stats.isFile() && supportedAudioExtensions.includes(path.extname(answer)))) {
                return true;
            }
            return "Invalid Directory";
        }
...
function resolveData(tag, token, guildID, channelID = [], wayNotify = [], musicPath, webhookURL, userNotify, captchaAPI, apiUser, apiKey, cmdPrefix, autoDaily, autoPray, autoSlash, autoGem, autoLootbox, gemOrder, autoQuote, autoOther, autoRefresh, autoSleep, autoResume, aliasName, timePray, huntPet, userIdPray, textOwo) {
    return {
        tag,
        token,
        guildID,
...
export async function collectData(data) {
    ...
    while (!client) {
        ...
        if (account === 0) {
            ...
            if (choice === 1) {
                await getResult(listCheckbox("list", "Press Enter to go back", ["Back"]), "Thank you for your generous support, we are truly grateful!\n\n   \x1b[1mMB Bank Vietnam / Momo / ZaloPay:\x1b[0m      NGUYEN THANH LONG     0978176370\n")
            }
        } else {
    ...
    if (waynotify.includes(3)) {
        ...
        while (true) {
            if (!musicpath || musicpath == "none") {
                ...
                musicpath = await getResult(music2(musicpath));
            } else {
                break;
            }
        }
    ...
    if (waynotify.includes(0) || waynotify.includes(1) || waynotify.includes(2)) {
    ...
    autoother = false
    autorefresh = false
    aliasName = await getResult(getAliasName(cache?.aliasName))
    autopray = await getResult(trueFalse("Toggle Automatically Pray", cache?.autoPray))
    if(autopray){
        userIdPray = await getResult(getUserPray(cache?.userIdPray))
        timePray = await getResult(getTimePray("5")) // default 5m
    }
    huntPet = await getResult(trueFalse("Do you hunt pet?", cache?.huntPet))
    autogem = false
    if(huntPet){
        autogem = await getResult(trueFalse("Toggle Automatically Use Hunt Gems", cache?.autoGem))
        if (autogem) {
            gemorder = await getResult(gemOrder(cache?.gemOrder))
        }
        if (autogem) {
            autolootbox = await getResult(trueFalse("Toggle Automatically Loot Boxes", cache?.autoLootbox))
        }
    }
    textOwo = await getResult(trueFalse("Do u want chat owo text? ", cache?.textOwo))
    ...
    conf = resolveData(client.user.username, client.token, guildid, channelid, waynotify, musicpath, webhookurl, usernotify, autocaptcha, apiuser, apikey, prefix, autodaily, autopray, autoslash, autogem, autolootbox, gemorder, autoquote, autoother, autorefresh, autosleep, autoresume, aliasName, timePray, huntPet, userIdPray, textOwo)
